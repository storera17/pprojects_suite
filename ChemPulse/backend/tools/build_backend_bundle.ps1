...
$ZipPath = Join-Path $Root "dist\chempulse-backend.zip"
Set-Location $Root
...
& $Python -m pip install -r requirement\backend.txt
...
Copy-Item -LiteralPath (Join-Path $Root "requirement\backend.txt") -Destination (Join-Path $DistRoot "backend-requirements.txt") -Force
...
Write-Host "Built backend bundle at $ZipPath"
